...
struct NexaMacCLI {
    static func main() async {
        ...
        default:
            fputs("nexa-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      nexa-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      nexa-mac discover [--timeout <ms>] [--json] [--include-local]
      nexa-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      nexa-mac connect
      nexa-mac connect --url ws://127.0.0.1:18789 --json
      nexa-mac discover --timeout 3000 --json
      nexa-mac wizard --mode local
    """)
}
