...
enum CommandResolver {
    private static let projectRootDefaultsKey = "nexa.gatewayProjectRootPath"
    private static let helperName = "nexa"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let nexaEntry = root.appendingPathComponent("nexa.mjs").path
        if FileManager().isReadableFile(atPath: nexaEntry) { return nexaEntry }
        let binEntry = root.appendingPathComponent("bin/nexa.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/nexa")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let nexaPaths = self.nexaManagedPaths(home: home)
        if !nexaPaths.isEmpty {
            extras.insert(contentsOf: nexaPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + nexaPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func nexaManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".nexa"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func nexaExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectNexaExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("nexa.mjs").path,
            root.appendingPathComponent("bin/nexa.js").path,
        ]
    ...
    static func hasAnyNexaInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.nexaExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let nexaPath = self.projectNexaExecutable(projectRoot: root) {
                return [nexaPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "nexa", subcommand] + extraArgs
            }
            if let nexaPath = self.nexaExecutable(searchPaths: searchPaths) {
                return [nexaPath, subcommand] + extraArgs
            }

            let missingEntry = """
            nexa entrypoint missing (looked for dist/index.js or nexa.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real nexa CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/nexa"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/nexa.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/nexa.mjs"
            node "$PRJ/nexa.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/nexa.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/nexa.js"
            node "$PRJ/bin/nexa.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent nexa"
          pnpm --silent nexa \(quotedArgs);
        else
          echo "nexa CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? NexaConfigFile.loadDict()
