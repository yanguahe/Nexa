...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.nexa", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> NexaSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: NexaSessionPreviewEntry,
    ...
    private static func cache(payload: NexaSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: NexaChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [NexaKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [NexaKit.AnyCodable]) -> [NexaChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(NexaChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: NexaChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: NexaChatMessage) -> Bool {
    ...
    private static func toolNames(for message: NexaChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: NexaChatMessage) -> String? {
