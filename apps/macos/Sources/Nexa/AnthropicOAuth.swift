...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (Nexa token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: NexaOAuthStore.AnthropicOAuthStatus = NexaOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.nexa", category: "anthropic-oauth")
...
enum NexaOAuthStore {
    ...
    private static let nexaOAuthDirEnv = "NEXA_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "Nexa OAuth token file not found"
            case .unreadableFile: "Nexa OAuth token file not readable"
            case .invalidJSON: "Nexa OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in Nexa OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "Nexa OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.nexaOAuthDirEnv]?
