...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.nexa", category: "gateway.env")
    private static let supportedBindModes: Set<String> = ["loopback", "tailnet", "lan", "auto"]

    static func gatewayPort() -> Int {
        if let raw = ProcessInfo.processInfo.environment["NEXA_GATEWAY_PORT"] {
            let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
            if let parsed = Int(trimmed), parsed > 0 { return parsed }
        }
        if let configPort = NexaConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.nexaExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "nexa CLI not found in PATH; install the CLI.")
            }
    ...
    private static func preferredGatewayBind() -> String? {
        ...
        if let env = ProcessInfo.processInfo.environment["NEXA_GATEWAY_BIND"] {
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "nexa@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "nexa@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "nexa@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "nexa@\(target)"])
            }

        statusHandler("Installing nexa@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed nexa@\(target)")
        } else {
