...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.nexa", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: NexaPeekabooBridgeServices?
    private static var nexaSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("Nexa", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = NexaPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.nexaSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.nexaSocketPath, privacy: .public)")
    }
...
@MainActor
private final class NexaPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.nexa.peekaboo")
