...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["NEXA_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/nexa")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("nexa-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("nexa-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/nexa/ (rolling or stdout), or nil if none exist.
