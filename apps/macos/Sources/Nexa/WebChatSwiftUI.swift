...
private let webChatSwiftLogger = Logger(subsystem: "ai.nexa", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: NexaChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> NexaChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(NexaChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> NexaChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                NexaGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<NexaChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any NexaChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = NexaChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: NexaChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<NexaChatView>) -> NSViewController
    {
