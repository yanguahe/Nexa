...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.nexa/nexa.json. Prefer NEXA_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.nexa/nexa.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.nexa/nexa.json. Restart the gateway to apply."
        }
