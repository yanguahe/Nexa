...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.nexa.mac`) even if the shipped
    ...
    static func resolveNixMode(
        ...
        if environment["NEXA_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "nexa.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "nexa.nixMode") { return true }
