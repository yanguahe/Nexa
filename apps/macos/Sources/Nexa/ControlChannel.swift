...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: NexaProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.nexa", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: NexaKit.AnyCodable]()) {
                $0[$1.key] = NexaKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: NexaProtocol.AnyCodable?) -> [String: NexaProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: NexaProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: NexaKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: NexaProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("nexa.control.heartbeat")
    static let controlAgentEvent = Notification.Name("nexa.control.agent")
}
