...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: NexaOAuthStore.AnthropicOAuthStatus = NexaOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: NexaOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = NexaOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = NexaOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try NexaOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. Nexa can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: NexaOAuthStore.AnthropicOAuthStatus,
