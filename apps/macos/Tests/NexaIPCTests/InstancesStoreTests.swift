...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: NexaProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: NexaProtocol.AnyCodable] = [
            "presence": .init([NexaProtocol.AnyCodable(entry)]),
        ]
        let payload = NexaProtocol.AnyCodable(payloadMap)
