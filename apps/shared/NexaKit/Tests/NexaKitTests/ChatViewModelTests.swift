...
private final class TestChatTransport: @unchecked Sendable, NexaChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [NexaChatHistoryPayload]
    private let sessionsResponses: [NexaChatSessionsListResponse]

    private let stream: AsyncStream<NexaChatTransportEvent>
    private let continuation: AsyncStream<NexaChatTransportEvent>.Continuation

    init(
        historyResponses: [NexaChatHistoryPayload],
        sessionsResponses: [NexaChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<NexaChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> NexaChatHistoryPayload {
        ...
        return self.historyResponses.last ?? NexaChatHistoryPayload(
        ...
        attachments _: [NexaChatAttachmentPayload]) async throws -> NexaChatSendResponse
    {
        ...
        return NexaChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> NexaChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? NexaChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: NexaChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = NexaChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = NexaChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { NexaChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = NexaChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = NexaChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = NexaChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { NexaChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
