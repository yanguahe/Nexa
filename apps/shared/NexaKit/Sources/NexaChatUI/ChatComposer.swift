...
@MainActor
struct NexaChatComposer: View {
    @Bindable var viewModel: NexaChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(NexaChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(NexaChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \NexaPendingAttachment.id)
                { (att: NexaPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            NexaPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(NexaChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(NexaChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(NexaChatTheme.subtleCard)
        .clipShape(Capsule())
    }
