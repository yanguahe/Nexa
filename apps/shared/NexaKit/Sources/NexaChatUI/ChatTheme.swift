...
enum NexaPlatformImageFactory {
    static func image(_ image: NexaPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
