...
struct ChatMessageBubble: View {
    let message: NexaChatMessage
    let style: NexaChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? NexaChatTheme.userText : NexaChatTheme.assistantText
    ...
    private var inlineAttachments: [NexaChatMessageContent] {
    ...
    private var toolCalls: [NexaChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [NexaChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? NexaChatTheme.userBubble
        }
        if self.style == .onboarding {
            return NexaChatTheme.onboardingAssistantBubble
        }
        return NexaChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return NexaChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: NexaChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? NexaChatTheme.userText : NexaChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: NexaChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(NexaChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [NexaChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: NexaChatTheme.assistantText)
            }
        }
    }
}
