...
private let chatUILogger = Logger(subsystem: "ai.nexa", category: "NexaChatUI")

@MainActor
@Observable
public final class NexaChatViewModel {
    public private(set) var messages: [NexaChatMessage] = []
    ...
    public var attachments: [NexaPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [NexaChatPendingToolCall] = []
    public private(set) var sessions: [NexaChatSessionEntry] = []
    private let transport: any NexaChatTransport
    ...
    private var pendingToolCallsById: [String: NexaChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any NexaChatTransport) {
    ...
    public var sessionChoices: [NexaChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [NexaChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [NexaChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: NexaPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [NexaChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: NexaChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [NexaChatMessage]) -> [NexaChatMessage] {
        var result: [NexaChatMessage] = []
    ...
    private static func dedupeKey(for message: NexaChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [NexaChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> NexaChatAttachmentPayload in
            NexaChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> NexaChatSessionEntry {
        NexaChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: NexaChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: NexaChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: NexaAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = NexaChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> NexaPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
