...
final class TalkModeManager: NSObject {
    ...
    private var pttCompletion: CheckedContinuation<NexaTalkPTTStopPayload, Never>?
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = NexaTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> NexaTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return NexaTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> NexaTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> NexaTalkPTTStopPayload {
    ...
    func cancelPushToTalk() async -> NexaTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = NexaTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: NexaTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(payload, as: NexaAgentEventPayload.self) else {
                continue
            }
