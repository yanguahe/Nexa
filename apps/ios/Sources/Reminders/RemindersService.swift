...
final class RemindersService: RemindersServicing {
    func list(params: NexaRemindersListParams) async throws -> NexaRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[NexaReminderPayload], Error>) in
        ...
        return NexaRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: NexaRemindersAddParams) async throws -> NexaRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = NexaReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return NexaRemindersAddPayload(reminder: payload)
    }
