...
final class MotionService: MotionServicing {
    func activities(params: NexaMotionActivityParams) async throws -> NexaMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[NexaMotionActivityEntry], Error>) in
        ...
        return NexaMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: NexaPedometerParams) async throws -> NexaPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<NexaPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = NexaPedometerPayload(
