...
final class ContactsService: ContactsServicing {
    ...
    func search(params: NexaContactsSearchParams) async throws -> NexaContactsSearchPayload {
        ...
        return NexaContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: NexaContactsAddParams) async throws -> NexaContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return NexaContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return NexaContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> NexaContactPayload {
        NexaContactPayload(
