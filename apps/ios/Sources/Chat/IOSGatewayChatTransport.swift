...
struct IOSGatewayChatTransport: NexaChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> NexaChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(NexaChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> NexaChatHistoryPayload {
        ...
        return try JSONDecoder().decode(NexaChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [NexaChatAttachmentPayload]) async throws -> NexaChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [NexaChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(NexaChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(NexaGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: NexaGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: NexaChatEventPayload.self)
                        {
