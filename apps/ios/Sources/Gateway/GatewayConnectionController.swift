...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [NexaCapability.canvas.rawValue, NexaCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(NexaCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(NexaCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = NexaLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(NexaCapability.location.rawValue) }

        caps.append(NexaCapability.device.rawValue)
        caps.append(NexaCapability.photos.rawValue)
        caps.append(NexaCapability.contacts.rawValue)
        caps.append(NexaCapability.calendar.rawValue)
        caps.append(NexaCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(NexaCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            NexaScreenCommand.record.rawValue,
            NexaSystemCommand.notify.rawValue,
            NexaChatCommand.push.rawValue,
        ...
        if caps.contains(NexaCapability.camera.rawValue) {
        ...
        if caps.contains(NexaCapability.location.rawValue) {
            commands.append(NexaLocationCommand.get.rawValue)
        }
        if caps.contains(NexaCapability.device.rawValue) {
            commands.append(NexaDeviceCommand.status.rawValue)
            commands.append(NexaDeviceCommand.info.rawValue)
        }
        if caps.contains(NexaCapability.photos.rawValue) {
            commands.append(NexaPhotosCommand.latest.rawValue)
        }
        if caps.contains(NexaCapability.contacts.rawValue) {
            commands.append(NexaContactsCommand.search.rawValue)
            commands.append(NexaContactsCommand.add.rawValue)
        }
        if caps.contains(NexaCapability.calendar.rawValue) {
            commands.append(NexaCalendarCommand.events.rawValue)
            commands.append(NexaCalendarCommand.add.rawValue)
        }
        if caps.contains(NexaCapability.reminders.rawValue) {
            commands.append(NexaRemindersCommand.list.rawValue)
            commands.append(NexaRemindersCommand.add.rawValue)
        }
        if caps.contains(NexaCapability.motion.rawValue) {
            commands.append(NexaMotionCommand.activity.rawValue)
            commands.append(NexaMotionCommand.pedometer.rawValue)
        }
