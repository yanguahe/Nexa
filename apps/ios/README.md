...
This is an **alpha** iOS app that connects to an Nexa Gateway as a `role: node`.
...
3. On iOS: Nexa → Settings → Gateway → paste setup code → Connect
...
cd apps/ios
xcodegen generate
xcodebuild test -project Nexa.xcodeproj -scheme Nexa -destination "platform=iOS Simulator,name=iPhone 17"
```

## Shared Code

- `apps/shared/NexaKit` contains the shared transport/types used by the iOS app.
