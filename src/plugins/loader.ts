...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: NexaPluginDefinition;
  register?: NexaPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as NexaPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as NexaPluginDefinition;
...
export function loadNexaPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverNexaPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias || pluginSdkAccountIdAlias
      ? {
          alias: {
            ...(pluginSdkAlias ? { "nexa/plugin-sdk": pluginSdkAlias } : {}),
            ...(pluginSdkAccountIdAlias
              ? { "nexa/plugin-sdk/account-id": pluginSdkAccountIdAlias }
              : {}),
          },
        }
      : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: NexaPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as NexaPluginModule;
    } catch (err) {
