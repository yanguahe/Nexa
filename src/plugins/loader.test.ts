...
const fixtureRoot = path.join(os.tmpdir(), `nexa-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.NEXA_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "nexa.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    ...
    process.env.NEXA_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadNexaPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.NEXA_BUNDLED_PLUGINS_DIR = bundledDir;
    ...
    const enabledRegistry = loadNexaPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
