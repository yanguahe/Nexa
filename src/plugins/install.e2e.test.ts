...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `nexa-plugin-install-${randomUUID()}`);
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.nexa/extensions and uses unscoped id", async () => {
  ...
  it("rejects packages without nexa.extensions", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
    const pkgDir = path.join(workDir, "package");
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@nexa/nope", version: "0.0.1" }),
      "utf-8",
    );
    ...
    expect(result.error).toContain("nexa.extensions");
  });
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(argv).toEqual(["npm", "pack", "@nexa/voice-call@0.0.1", "--ignore-scripts"]);
