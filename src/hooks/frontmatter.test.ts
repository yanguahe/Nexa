...
`;
    ...
    expect(parsed.nexa.emoji).toBe("💾");
    expect(parsed.nexa.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.nexa.emoji).toBe("📝");
    expect(parsed.nexa.events).toEqual(["command"]);
    expect(parsed.nexa.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.nexa.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"nexa": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"nexa": {"events": ["test"]}}');
  });
...
describe("resolveNexaMetadata", () => {
  it("extracts nexa metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveNexaMetadata(frontmatter);
  ...
  it("returns undefined when nexa key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        nexa: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Nexa" },
            { id: "npm", kind: "npm", package: "@nexa/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@nexa/hook");
  });
...
`;
    ...
    const nexa = resolveNexaMetadata(frontmatter);
    expect(nexa).toBeDefined();
    expect(nexa?.emoji).toBe("💾");
    expect(nexa?.events).toEqual(["command:new"]);
    expect(nexa?.requires?.config).toEqual(["workspace.dir"]);
    expect(nexa?.install?.[0].kind).toBe("bundled");
  });
