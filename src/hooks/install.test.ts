...
const fixtureRoot = path.join(os.tmpdir(), `nexa-hook-install-${randomUUID()}`);
let tempDirIndex = 0;
...
beforeAll(async () => {
  ...
  zipHooks.file(
    ...
    [
      ...
      'metadata: {"nexa":{"events":["command:new"]}}',
      "---",
      "",
      "# Zip Hook",
    ].join("\n"),
  ...
  const makeTarWithPackage = async (params: {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "0.0.1",
        nexa: { hooks: [`./hooks/${params.hookName}`] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
  ...
  const makeNpmPackTgz = async (): Promise<Buffer> => {
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({
        name: "@nexa/test-hooks",
        version: "0.0.1",
        nexa: { hooks: ["./hooks/one-hook"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
...
describe("installHooksFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(argv).toEqual(["npm", "pack", "@nexa/test-hooks@0.0.1", "--ignore-scripts"]);
