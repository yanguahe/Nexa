...
This directory contains hooks that ship with Nexa. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.nexa/nexa.json`:
...
2. Restart gateway: `pkill -9 -f 'nexa.*gateway' && pnpm nexa gateway`
3. Enable the hook: `nexa hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.nexa.ai/automation/hooks
