...
homepage: https://docs.nexa.ai/automation/hooks#command-logger
metadata:
  {
    "nexa":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.nexa/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.nexa/logs/commands.log ~/.nexa/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/nexa`:
   ```
   /home/username/.nexa/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
