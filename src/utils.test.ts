...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "nexa-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "nexa-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "nexa-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "nexa-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.nexa when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "nexa-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers NEXA_HOME over HOME", () => {
    vi.stubEnv("NEXA_HOME", "/srv/nexa-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/nexa-home"));
...
describe("shortenHomePath", () => {
  it("uses $NEXA_HOME prefix when NEXA_HOME is set", () => {
    vi.stubEnv("NEXA_HOME", "/srv/nexa-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/nexa-home")}/.nexa/nexa.json`)).toBe(
      "$NEXA_HOME/.nexa/nexa.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $NEXA_HOME replacement when NEXA_HOME is set", () => {
    vi.stubEnv("NEXA_HOME", "/srv/nexa-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/nexa-home")}/.nexa/nexa.json`),
    ).toBe("config: $NEXA_HOME/.nexa/nexa.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/nexa")).toBe(path.resolve(os.homedir(), "nexa"));
  });
  ...
  it("prefers NEXA_HOME for tilde expansion", () => {
    vi.stubEnv("NEXA_HOME", "/srv/nexa-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/nexa")).toBe(path.resolve("/srv/nexa-home", "nexa"));
