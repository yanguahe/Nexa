...
describe("canvas host state dir defaults", () => {
  ...
  it("uses NEXA_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nexa-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("Nexa Canvas");
    } finally {
      await handler.close();
