...
import type { NexaConfig } from "../config/types.js";
import type { ConsoleStyle } from "./console.js";
import { resolvePreferredNexaTmpDir } from "../infra/tmp-nexa-dir.js";
import { readLoggingConfig } from "./config.js";
import { type LogLevel, levelToMinLevel, normalizeLogLevel } from "./levels.js";
import { loggingState } from "./state.js";

export const DEFAULT_LOG_DIR = resolvePreferredNexaTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "nexa.log"); // legacy single-file path

const LOG_PREFIX = "nexa";
...
function resolveSettings(): ResolvedSettings {
  let cfg: NexaConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig("../config/config.js") as {
        loadConfig?: () => NexaConfig;
      };
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.NEXA_TEST_FILE_LOG !== "1" ? "silent" : "info";
