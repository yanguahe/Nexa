...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".nexa");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.NEXA_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$NEXA_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via NEXA_STATE_DIR.
