...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "nexa-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.NEXA_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "nexa");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.NEXA_STATE_DIR = stateDir;
    process.env.NEXA_CONFIG_PATH = path.join(stateDir, "nexa.json");
