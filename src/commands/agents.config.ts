...
export function listAgentEntries(cfg: NexaConfig): AgentEntry[] {
...
function resolveAgentName(cfg: NexaConfig, agentId: string) {
...
function resolveAgentModel(cfg: NexaConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: NexaConfig): AgentSummary[] {
