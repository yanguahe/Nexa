...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.NEXA_DEBUG_HEALTH)) {
...
export async function healthCommand(
  opts: { json?: boolean; timeoutMs?: number; verbose?: boolean; config?: NexaConfig },
  runtime: RuntimeEnv,
) {
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.NEXA_DEBUG_HEALTH);
    const rich = isRich();
    if (opts.verbose) {
