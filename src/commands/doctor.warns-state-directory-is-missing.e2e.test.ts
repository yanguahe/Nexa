...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "nexa-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.NEXA_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when NEXA_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/nexa.json",
      exists: true,
      raw: "{}",
    ...
    process.env.NEXA_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
