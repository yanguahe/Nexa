...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "NEXA_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("NEXA_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv NEXA_GATEWAY_TOKEN");
    expect(message).not.toContain("NEXA_GATEWAY_PASSWORD");
  });
