...
beforeAll(() => {
  previousProfile = process.env.NEXA_PROFILE;
  process.env.NEXA_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.NEXA_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/nexa-root.js", () => ({
  resolveNexaPackageRoot: vi.fn().mockResolvedValue("/tmp/nexa"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/nexa/pnpm-lock.yaml",
      markerPath: "/tmp/nexa/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = (runtime.log as vi.Mock).mock.calls.map((c) => String(c[0]));
    expect(logs.some((l) => l.includes("Nexa status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("nexa status --all") ||
          l.includes("nexa --profile isolated status --all") ||
          l.includes("nexa status --all") ||
          l.includes("nexa --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.NEXA_GATEWAY_TOKEN = "abcd1234";
    try {
