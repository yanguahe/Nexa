...
export async function updateConfig(
  mutator: (cfg: NexaConfig) => NexaConfig,
): Promise<NexaConfig> {
...
export function resolveModelTarget(params: { raw: string; cfg: NexaConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: NexaConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("nexa agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
