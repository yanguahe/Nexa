...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveNexaAgentDir: vi.fn().mockReturnValue("/tmp/nexa-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/nexa-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/nexa-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveNexaAgentDir: mocks.resolveNexaAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveNexaAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/nexa-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/nexa-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/nexa-agent-custom");
