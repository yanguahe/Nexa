...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveNexaAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("nexa models auth setup-token")}\` or \`${formatCliCommand("nexa configure")}\`.`
          : `Run \`${formatCliCommand("nexa configure")}\` or set an API key env var.`;
