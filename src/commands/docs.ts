...
const SEARCH_TOOL = "https://docs.nexa.ai/mcp.SearchNexa";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.nexa.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('nexa docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.nexa.ai/");
      runtime.log(`Search: ${formatCliCommand('nexa docs "your query"')}`);
    }
    return;
  }
