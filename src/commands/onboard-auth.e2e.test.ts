...
const requireAgentDir = () => {
  const agentDir = process.env.NEXA_AGENT_DIR;
  if (!agentDir) {
    throw new Error("NEXA_AGENT_DIR not set");
  }
  return agentDir;
};
...
describe("writeOAuthCredentials", () => {
  ...
  afterEach(async () => {
    ...
    delete process.env.NEXA_OAUTH_DIR;
  });

  it("writes auth-profiles.json under NEXA_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "nexa-oauth-"));
    process.env.NEXA_STATE_DIR = tempStateDir;
    process.env.NEXA_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.NEXA_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to NEXA_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "nexa-minimax-"));
    process.env.NEXA_STATE_DIR = tempStateDir;
    process.env.NEXA_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.NEXA_AGENT_DIR;
