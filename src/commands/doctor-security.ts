...
import type { NexaConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: NexaConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("nexa security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("nexa configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("nexa config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("nexa doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("nexa config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('nexa config set session.dmScope "per-channel-peer"') +
