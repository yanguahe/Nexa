...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: NexaConfig | null = null;
  ...
  get(key: string, cfg: NexaConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: NexaConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: NexaConfig): void {
