...
import { ensureNexaCliOnPath } from "./path-env.js";

describe("ensureNexaCliOnPath", () => {
  let fixtureRoot = "";
  let fixtureCount = 0;
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nexa-path-"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling nexa exists", async () => {
    const tmp = await makeTmpDir();
    const appBinDir = path.join(tmp, "AppBin");
    await fs.mkdir(appBinDir);
    const cliPath = path.join(appBinDir, "nexa");
    await fs.writeFile(cliPath, "#!/bin/sh\necho ok\n", "utf-8");
    await fs.chmod(cliPath, 0o755);

    const originalPath = process.env.PATH;
    const originalFlag = process.env.NEXA_PATH_BOOTSTRAPPED;
    process.env.PATH = "/usr/bin";
    delete process.env.NEXA_PATH_BOOTSTRAPPED;
    try {
      ensureNexaCliOnPath({
        execPath: cliPath,
        cwd: tmp,
      ...
      if (originalFlag === undefined) {
        ...
        process.env.NEXA_PATH_BOOTSTRAPPED = originalFlag;
      }
    }
  });
  ...
  it("prepends mise shims when available", async () => {
    ...
    try {
      const appBinDir = path.join(tmp, "AppBin");
      await fs.mkdir(appBinDir);
      const appCli = path.join(appBinDir, "nexa");
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", async () => {
    ...
    try {
      ...
      const localCli = path.join(localBinDir, "nexa");
