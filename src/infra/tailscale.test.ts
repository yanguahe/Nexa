...
describe("tailscale helpers", () => {
  const originalForcedBinary = process.env.NEXA_TEST_TAILSCALE_BINARY;

  beforeEach(() => {
    process.env.NEXA_TEST_TAILSCALE_BINARY = "tailscale";
  });

  afterEach(() => {
    if (originalForcedBinary === undefined) {
      delete process.env.NEXA_TEST_TAILSCALE_BINARY;
    } else {
      process.env.NEXA_TEST_TAILSCALE_BINARY = originalForcedBinary;
    }
    vi.restoreAllMocks();
  });
