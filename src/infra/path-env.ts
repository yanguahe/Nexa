...
function candidateBinDirs(opts: EnsureNexaPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `nexa` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "nexa");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.NEXA_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "nexa"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `nexa` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureNexaCliOnPath(opts: EnsureNexaPathOpts = {}) {
  if (isTruthyEnvValue(process.env.NEXA_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.NEXA_PATH_BOOTSTRAPPED = "1";
