...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.NEXA_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*nexa-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "nexa";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "nexa");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "nexa-gw");
  ...
  records.push(`_nexa-gw._tcp IN PTR ${instanceLabel}._nexa-gw._tcp`);
  records.push(`${instanceLabel}._nexa-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._nexa-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; nexa-content-hash: ${contentHash}\n${contentBody}`;
}
