...
export function triggerNexaRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.NEXA_SYSTEMD_UNIT,
        process.env.NEXA_PROFILE,
      );
  ...
  const label =
    process.env.NEXA_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.NEXA_PROFILE);
