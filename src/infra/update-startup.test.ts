...
describe("update-startup", () => {
  ...
  let resolveNexaPackageRoot: (typeof import("./nexa-root.js"))["resolveNexaPackageRoot"];
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nexa-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    hadStateDir = Object.prototype.hasOwnProperty.call(process.env, "NEXA_STATE_DIR");
    prevStateDir = process.env.NEXA_STATE_DIR;
    process.env.NEXA_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveNexaPackageRoot } = await import("./nexa-root.js"));
  ...
  it("logs update hint for npm installs when newer tag exists", async () => {
    vi.mocked(resolveNexaPackageRoot).mockResolvedValue("/opt/nexa");
