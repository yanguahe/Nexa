...
describe("Ghost reminder bug (issue #13317)", () => {
  const createConfig = async (
    tmpDir: string,
  ): Promise<{ cfg: NexaConfig; sessionKey: string }> => {
  ...
  it("does not use CRON_EVENT_PROMPT when only a HEARTBEAT_OK event is present", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nexa-ghost-"));
  ...
  it("uses CRON_EVENT_PROMPT when an actionable cron event exists", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nexa-cron-"));
    const sendTelegram = vi.fn().mockResolvedValue({
      messageId: "m1",
      chatId: "155462274",
    });
  ...
  it("uses CRON_EVENT_PROMPT when cron events are mixed with heartbeat noise", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nexa-cron-mixed-"));
    const sendTelegram = vi.fn().mockResolvedValue({
      messageId: "m1",
      chatId: "155462274",
    });
