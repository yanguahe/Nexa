...
describe("resolveEffectiveHomeDir", () => {
  it("prefers NEXA_HOME over HOME and USERPROFILE", () => {
    const env = {
  ...
  it("expands NEXA_HOME when set to ~", () => {
    const env = {
      NEXA_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for NEXA_HOME", () => {
    const result = resolveRequiredHomeDir(
      { NEXA_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when NEXA_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ NEXA_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { NEXA_HOME: "/srv/nexa-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/nexa-home")}/x`);
  });
