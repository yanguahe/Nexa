...
const DEFAULT_PACKAGE_NAME = "nexa";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nexa-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "nexa.mjs");
    ...
    const doctorStep = await runStep(
      step("nexa doctor", doctorArgv, gitRoot, { NEXA_UPDATE_IN_PROGRESS: "1" }),
    );
