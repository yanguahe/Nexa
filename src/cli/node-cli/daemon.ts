...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("nexa node install"), formatCliCommand("nexa node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("nexa node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    NEXA_LOG_PREFIX: baseEnv.NEXA_LOG_PREFIX ?? "node",
