...
vi.mock("../infra/nexa-root.js", () => ({
  resolveNexaPackageRoot: vi.fn(),
}));
...
const { resolveNexaPackageRoot } = await import("../infra/nexa-root.js");
...
describe("update-cli", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nexa-update-tests-"));
  });
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = await createCaseDir("nexa-update");
    setTty(false);
    readPackageVersion.mockResolvedValue("2.0.0");

    vi.mocked(resolveNexaPackageRoot).mockResolvedValue(tempDir);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveNexaPackageRoot).mockReset();
    ...
    vi.mocked(resolveNexaPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("nexa");
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = await createCaseDir("nexa-update-wizard");
    const previousGitDir = process.env.NEXA_GIT_DIR;
    try {
      setTty(true);
      process.env.NEXA_GIT_DIR = tempDir;
      ...
      process.env.NEXA_GIT_DIR = previousGitDir;
    }
  });
});
