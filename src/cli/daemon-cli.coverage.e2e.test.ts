...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    NEXA_STATE_DIR: process.env.NEXA_STATE_DIR,
    NEXA_CONFIG_PATH: process.env.NEXA_CONFIG_PATH,
    NEXA_GATEWAY_PORT: process.env.NEXA_GATEWAY_PORT,
    NEXA_PROFILE: process.env.NEXA_PROFILE,
  };

  beforeEach(() => {
    process.env.NEXA_STATE_DIR = "/tmp/nexa-cli-state";
    process.env.NEXA_CONFIG_PATH = "/tmp/nexa-cli-state/nexa.json";
    delete process.env.NEXA_GATEWAY_PORT;
    delete process.env.NEXA_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.NEXA_STATE_DIR !== undefined) {
      process.env.NEXA_STATE_DIR = originalEnv.NEXA_STATE_DIR;
    } else {
      delete process.env.NEXA_STATE_DIR;
    }

    if (originalEnv.NEXA_CONFIG_PATH !== undefined) {
      process.env.NEXA_CONFIG_PATH = originalEnv.NEXA_CONFIG_PATH;
    } else {
      delete process.env.NEXA_CONFIG_PATH;
    }

    if (originalEnv.NEXA_GATEWAY_PORT !== undefined) {
      process.env.NEXA_GATEWAY_PORT = originalEnv.NEXA_GATEWAY_PORT;
    } else {
      delete process.env.NEXA_GATEWAY_PORT;
    }

    if (originalEnv.NEXA_PROFILE !== undefined) {
      process.env.NEXA_PROFILE = originalEnv.NEXA_PROFILE;
    } else {
      delete process.env.NEXA_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        NEXA_PROFILE: "dev",
        NEXA_STATE_DIR: "/tmp/nexa-daemon-state",
        NEXA_CONFIG_PATH: "/tmp/nexa-daemon-state/nexa.json",
        NEXA_GATEWAY_PORT: "19001",
      },
