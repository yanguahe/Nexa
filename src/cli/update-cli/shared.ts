...
import { resolveNexaPackageRoot } from "../../infra/nexa-root.js";
...
const NEXA_REPO_URL = "https://github.com/nexa/nexa.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "nexa";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("nexa@")) {
    return trimmed.slice("nexa@".length);
  }
...
export function resolveGitInstallDir(): string {
  const override = process.env.NEXA_GIT_DIR?.trim();
  if (override) {
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", NEXA_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `NEXA_GIT_DIR points at a non-git directory: ${params.dir}. Set NEXA_GIT_DIR to an empty folder or an nexa checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`NEXA_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "nexa.mjs");
