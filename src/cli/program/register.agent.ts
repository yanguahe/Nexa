...
${formatHelpExamples([
  ['nexa agent --to +15555550123 --message "status update"', "Start a new session."],
  ['nexa agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'nexa agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'nexa agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['nexa agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.nexa.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.nexa.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['nexa agents set-identity --agent main --name "Nexa" --emoji "🦞"', "Set name + emoji."],
  ["nexa agents set-identity --agent main --avatar avatars/nexa.png", "Set avatar path."],
  [
    "nexa agents set-identity --workspace ~/.nexa/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "nexa agents set-identity --identity-file ~/.nexa/workspace/IDENTITY.md --agent main",
