...
const SANDBOX_EXAMPLES = {
  main: [
    ["nexa sandbox list", "List all sandbox containers."],
    ["nexa sandbox list --browser", "List only browser containers."],
    ["nexa sandbox recreate --all", "Recreate all containers."],
    ["nexa sandbox recreate --session main", "Recreate a specific session."],
    ["nexa sandbox recreate --agent mybot", "Recreate agent containers."],
    ["nexa sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["nexa sandbox list", "List all sandbox containers."],
    ["nexa sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["nexa sandbox recreate --all", "Recreate all containers."],
    ["nexa sandbox recreate --session main", "Recreate a specific session."],
    ["nexa sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["nexa sandbox recreate --browser --all", "Recreate only browser containers."],
    ["nexa sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["nexa sandbox explain", "Show effective sandbox config."],
    ["nexa sandbox explain --session agent:main:main", "Explain a specific session."],
    ["nexa sandbox explain --agent work", "Explain an agent sandbox."],
    ["nexa sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.nexa.ai/cli/sandbox")}\n`,
    )
    .action(() => {
