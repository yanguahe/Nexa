...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "nexa-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...
      NEXA_SKIP_GMAIL_WATCHER: "1",
      NEXA_SKIP_CRON: "1",
      NEXA_SKIP_BROWSER_CONTROL_SERVER: "1",
      NEXA_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "nexa-entry-bootstrap.mjs");
