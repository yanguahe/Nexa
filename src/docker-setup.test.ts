...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "nexa-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    NEXA_GATEWAY_TOKEN: "test-token",
    NEXA_CONFIG_DIR: join(sandbox.rootDir, "config"),
    NEXA_WORKSPACE_DIR: join(sandbox.rootDir, "nexa"),
  };
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = spawnSync("bash", [sandbox.scriptPath], {
      ...
      env: createEnv(sandbox, {
        NEXA_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
        NEXA_EXTRA_MOUNTS: undefined,
        NEXA_HOME_VOLUME: "nexa-home",
      }),
      stdio: ["ignore", "ignore", "pipe"],
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(sandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("NEXA_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("NEXA_EXTRA_MOUNTS=");
    expect(envFile).toContain("NEXA_HOME_VOLUME=nexa-home");
    const extraCompose = await readFile(join(sandbox.rootDir, "docker-compose.extra.yml"), "utf8");
    expect(extraCompose).toContain("nexa-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("nexa-home:");
    const log = await readFile(sandbox.logPath, "utf8");
    expect(log).toContain("--build-arg NEXA_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
