...
function listAgents(cfg: NexaConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: NexaConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: NexaConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<NexaConfig, EvaluatedBindingsCache>();
