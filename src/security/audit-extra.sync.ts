...
function summarizeGroupPolicy(cfg: NexaConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: NexaConfig): boolean {
...
function collectModels(cfg: NexaConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function listKnownNodeCommands(cfg: NexaConfig): Set<string> {
  const baseCfg: NexaConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: NexaConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: NexaConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: NexaConfig): boolean {
...
function isBrowserEnabled(cfg: NexaConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: NexaConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: NexaConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep NEXA_STATE_DIR on a local-only volume and re-run "${formatCliCommand("nexa security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: NexaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer NEXA_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const nexaGatewayToken =
    typeof env.NEXA_GATEWAY_TOKEN === "string" && env.NEXA_GATEWAY_TOKEN.trim()
      ? env.NEXA_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-nexa-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: NexaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectNodeDenyCommandPatternFindings(cfg: NexaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: NexaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: NexaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: NexaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
