...
function resolveExecConfig(params: { cfg?: NexaConfig; agentId?: string }) {
...
function resolveFsConfig(params: { cfg?: NexaConfig; agentId?: string }) {
  const cfg = params.cfg;
...
export function createNexaCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createNexaReadTool(freshReadTool);
