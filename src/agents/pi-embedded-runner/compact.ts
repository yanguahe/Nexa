...
import { resolveNexaDocsPath } from "../docs-path.js";
...
export async function compactEmbeddedPiSessionDirect(
  ...
  const agentDir = params.agentDir ?? resolveNexaAgentDir();
  await ensureNexaModelsJson(params.config, agentDir);
  ...
  try {
    ...
    const toolsRaw = createNexaCodingTools({
      exec: {
    ...
    const docsPath = await resolveNexaDocsPath({
      workspaceDir: effectiveWorkspace,
