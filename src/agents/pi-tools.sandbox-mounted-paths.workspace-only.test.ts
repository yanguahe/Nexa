...
describe("tools.fs.workspaceOnly", () => {
  it("defaults to allowing sandbox mounts outside the workspace root", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "nexa-sbx-mounts-"));
    ...
    try {
      ...
      const tools = createNexaCodingTools({ sandbox, workspaceDir: sandboxRoot });
  ...
  it("rejects sandbox mounts outside the workspace root when enabled", async () => {
    ...
    try {
      ...
      const cfg = { tools: { fs: { workspaceOnly: true } } } as unknown as NexaConfig;
      const tools = createNexaCodingTools({ sandbox, workspaceDir: sandboxRoot, config: cfg });
      const readTool = tools.find((tool) => tool.name === "read");
      const writeTool = tools.find((tool) => tool.name === "write");
