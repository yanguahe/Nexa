...
export function listAgentIds(cfg: NexaConfig): string[] {
...
export function resolveDefaultAgentId(cfg: NexaConfig): string {
  const agents = listAgents(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
export function resolveSessionAgentIds(params: { sessionKey?: string; config?: NexaConfig }): {
...
export function resolveAgentModelPrimary(cfg: NexaConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: NexaConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
...
export function resolveAgentDir(cfg: NexaConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
