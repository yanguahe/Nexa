...
describe("image tool implicit imageModel config", () => {
  ...
  it("stays disabled without auth when no pairing is possible", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "nexa-image-"));
  ...
  it("allows workspace images outside default local media roots", async () => {
    const workspaceParent = await fs.mkdtemp(
      path.join(process.cwd(), ".nexa-workspace-image-"),
    );
    try {
  ...
  it("allows workspace images via createNexaCodingTools default workspace root", async () => {
    const workspaceParent = await fs.mkdtemp(
      path.join(process.cwd(), ".nexa-workspace-image-"),
    );
    try {
      ...
      const tools = createNexaCodingTools({ config: cfg, agentDir });
  ...
  it("sandboxes image paths like the read tool", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "nexa-image-sandbox-"));
  ...
  it("rewrites inbound absolute paths into sandbox media/inbound", async () => {
    ...
    const res = await tool.execute("t1", {
      prompt: "Describe the image.",
      image: "@/Users/steipete/.nexa/media/inbound/photo.png",
    });
...
describe("image tool MiniMax VLM routing", () => {
  ...
  it("calls /v1/coding_plan/vlm for minimax image models", async () => {
    ...
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "nexa-minimax-vlm-"));
    vi.stubEnv("MINIMAX_API_KEY", "minimax-test");
    const cfg: NexaConfig = {
      agents: { defaults: { model: { primary: "minimax/MiniMax-M2.1" } } },
    };
