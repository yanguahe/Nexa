...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses NEXA_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      NEXA_HOME: "/srv/nexa-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/nexa-home"), ".nexa", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".nexa", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("nexa-workspace-");
