...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("nexa-ws-", async (workspaceDir) => {
      await withTempDir("nexa-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createNexaCodingTools({ workspaceDir });
    ...
    await withTempDir("nexa-ws-", async (workspaceDir) => {
      await withTempDir("nexa-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello nexa");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("nexa-ws-", async (workspaceDir) => {
      await withTempDir("nexa-override-", async (overrideDir) => {
        const tools = createNexaCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("nexa-sandbox-", async (sandboxDir) => {
      await withTempDir("nexa-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
        ...
        const tools = createNexaCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
