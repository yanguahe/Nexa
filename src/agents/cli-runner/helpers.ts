...
/**
 * Cleanup suspended Nexa CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: NexaConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "nexa-cli-images-"));
  const paths: string[] = [];
