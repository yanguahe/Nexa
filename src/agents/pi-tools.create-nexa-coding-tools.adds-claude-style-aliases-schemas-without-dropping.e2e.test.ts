...
import { __testing, createNexaCodingTools } from "./pi-tools.js";
...
describe("createNexaCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createNexaCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nexa-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "nexa-outside.txt");
