...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "nexa.sandbox=1");
  args.push("--label", `nexa.sessionKey=${params.scopeKey}`);
  args.push("--label", `nexa.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `nexa.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "nexa.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`nexa sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`nexa sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("nexa sandbox recreate --all");
}
