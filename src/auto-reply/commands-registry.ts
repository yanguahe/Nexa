...
export function isCommandEnabled(cfg: NexaConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: NexaConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as NexaConfig),
...
export function getCommandDetection(_cfg?: NexaConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: NexaConfig) {
