...
function snapshotStateEnv(): StateEnvSnapshot {
  return { NEXA_STATE_DIR: process.env.NEXA_STATE_DIR };
}

function restoreStateEnv(snapshot: StateEnvSnapshot) {
  if (snapshot.NEXA_STATE_DIR === undefined) {
    delete process.env.NEXA_STATE_DIR;
  } else {
    process.env.NEXA_STATE_DIR = snapshot.NEXA_STATE_DIR;
  }
}
...
describe("runReplyAgent typing (heartbeat)", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "nexa-typing-heartbeat-"));
  });
