...
import { scheduleGatewaySigusr1Restart, triggerNexaRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Nexa in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerNexaRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Nexa via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
