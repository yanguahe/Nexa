...
export function buildServiceEnvironment(params: {
  ...
  const stateDir = env.NEXA_STATE_DIR;
  const configPath = env.NEXA_CONFIG_PATH;
  return {
    HOME: env.HOME,
    PATH: buildMinimalServicePath({ env }),
    NEXA_PROFILE: profile,
    NEXA_STATE_DIR: stateDir,
    NEXA_CONFIG_PATH: configPath,
    NEXA_GATEWAY_PORT: String(port),
    NEXA_GATEWAY_TOKEN: token,
    NEXA_LAUNCHD_LABEL: resolvedLaunchdLabel,
    NEXA_SYSTEMD_UNIT: systemdUnit,
    NEXA_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    NEXA_SERVICE_KIND: GATEWAY_SERVICE_KIND,
