...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.NEXA_GATEWAY_PORT).toBe("18789");
    expect(env.NEXA_GATEWAY_TOKEN).toBe("secret");
    expect(env.NEXA_SERVICE_MARKER).toBe("nexa");
    expect(env.NEXA_SERVICE_KIND).toBe("gateway");
    expect(typeof env.NEXA_SERVICE_VERSION).toBe("string");
    expect(env.NEXA_SYSTEMD_UNIT).toBe("nexa-gateway.service");
    if (process.platform === "darwin") {
      expect(env.NEXA_LAUNCHD_LABEL).toBe("ai.nexa.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", NEXA_PROFILE: "work" },
      port: 18789,
    });
    expect(env.NEXA_SYSTEMD_UNIT).toBe("nexa-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.NEXA_LAUNCHD_LABEL).toBe("ai.nexa.work");
    }
  });
});
