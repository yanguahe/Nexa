import type { BrowserConfig, BrowserProfileConfig, NexaConfig } from "../config/config.js";
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "nexa" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_NEXA_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "nexa" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_NEXA_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_NEXA_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Nexa browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for nexa/nexa CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: NexaConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_NEXA_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "nexa";
