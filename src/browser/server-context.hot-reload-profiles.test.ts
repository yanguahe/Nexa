...
vi.mock("./chrome.js", () => ({
  ...
  resolveNexaUserDataDir: vi.fn(() => "/tmp/nexa"),
...
describe("server-context hot-reload profiles", () => {
  ...
  it("forProfile hot-reloads newly added profiles from config", async () => {
    ...
    // 2. Simulate adding a new profile to config (like user editing nexa.json)
  ...
  it("forProfile refreshes existing profile config after loadConfig cache updates", async () => {
    ...
    const before = ctx.forProfile("nexa");
    expect(before.profile.cdpPort).toBe(18800);

    cfgProfiles.nexa = { cdpPort: 19999, color: "#FF4500" };
    cachedConfig = null;

    const after = ctx.forProfile("nexa");
    expect(after.profile.cdpPort).toBe(19999);
    expect(state.resolved.profiles.nexa?.cdpPort).toBe(19999);
  });
