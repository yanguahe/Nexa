  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        defaultProfile: "nexa",
        profiles: {
          nexa: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    process.env.NEXA_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.NEXA_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.NEXA_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (prevGatewayPassword === undefined) {
      ...
      process.env.NEXA_GATEWAY_PASSWORD = prevGatewayPassword;
    }
