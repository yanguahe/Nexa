...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredNexaTmpDir: vi.fn(() => "/tmp/nexa"),
}));
vi.mock("../infra/tmp-nexa-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredNexaTmpDir.mockReturnValue("/tmp/nexa");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    ...
    tmpDirMocks.resolvePreferredNexaTmpDir.mockReturnValue("/tmp/nexa-preferred");
    setPwToolsCoreCurrentPage({ on, off });
    ...
    const expectedDownloadsTail = `${path.join("tmp", "nexa-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredNexaTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "nexa-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "nexa-preferred", "downloads")}${path.sep}`),
    );
  });
