...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/nexa" }));

beforeAll(async () => {
  chromeUserDataDir.dir = await fs.mkdtemp(path.join(os.tmpdir(), "nexa-chrome-user-data-"));
});
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        defaultProfile: "nexa",
        profiles: {
          nexa: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchNexaChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveNexaUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopNexaChrome: vi.fn(async () => {
...
export function installBrowserControlServerHooks() {
  beforeEach(async () => {
    ...
    state.prevGatewayPort = process.env.NEXA_GATEWAY_PORT;
    process.env.NEXA_GATEWAY_PORT = String(state.testPort - 2);
    ...
    state.prevGatewayToken = process.env.NEXA_GATEWAY_TOKEN;
    state.prevGatewayPassword = process.env.NEXA_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (state.prevGatewayPort === undefined) {
      ...
      process.env.NEXA_GATEWAY_PORT = state.prevGatewayPort;
    }
    if (state.prevGatewayToken === undefined) {
      delete process.env.NEXA_GATEWAY_TOKEN;
    } else {
      process.env.NEXA_GATEWAY_TOKEN = state.prevGatewayToken;
    }
    if (state.prevGatewayPassword === undefined) {
      delete process.env.NEXA_GATEWAY_PASSWORD;
    } else {
      process.env.NEXA_GATEWAY_PASSWORD = state.prevGatewayPassword;
    }
