...
function withLoopbackBrowserAuthImpl(
  ...
  if (headers.has("authorization") || headers.has("x-nexa-password")) {
    return { ...init, headers };
  }
  ...
  try {
    ...
    if (auth.password) {
      headers.set("x-nexa-password", auth.password);
      return { ...init, headers };
    }
  } catch {
  ...
  try {
    ...
    if (bridgeAuth?.token) {
      ...
      headers.set("x-nexa-password", bridgeAuth.password);
    }
  } catch {
    // ignore
  }
...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  const hint = isAbsoluteHttp(url)
    ...
    : `Start (or restart) the Nexa gateway (Nexa.app menubar, or \`${formatCliCommand("nexa gateway")}\`) and try again.`;
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the Nexa browser control service (timed out after ${timeoutMs}ms). ${hint}`,
    );
  }
  return new Error(`Can't reach the Nexa browser control service. ${hint} (${msg})`);
}
