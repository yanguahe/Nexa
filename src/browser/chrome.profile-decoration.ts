...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".nexa-profile-decorated");
}
 ...
 */
export function decorateNexaProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_NEXA_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_NEXA_BROWSER_COLOR).toUpperCase();
