...
export function resolveHooksConfig(cfg: NexaConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: NexaConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-nexa-token"] === "string"
      ? req.headers["x-nexa-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
