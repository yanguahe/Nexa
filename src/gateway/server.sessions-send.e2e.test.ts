...
import { createNexaTools } from "../agents/nexa-tools.js";
...
beforeAll(async () => {
  prevGatewayPort = process.env.NEXA_GATEWAY_PORT;
  prevGatewayToken = process.env.NEXA_GATEWAY_TOKEN;
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.NEXA_GATEWAY_PORT = String(gatewayPort);
  process.env.NEXA_GATEWAY_TOKEN = gatewayToken;
...
afterAll(async () => {
  ...
  if (prevGatewayPort === undefined) {
    ...
    process.env.NEXA_GATEWAY_PORT = prevGatewayPort;
  }
  if (prevGatewayToken === undefined) {
    delete process.env.NEXA_GATEWAY_TOKEN;
  } else {
    process.env.NEXA_GATEWAY_TOKEN = prevGatewayToken;
  }
});
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createNexaTools().find((candidate) => candidate.name === "sessions_send");
    if (!tool) {
