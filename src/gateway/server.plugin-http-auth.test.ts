...
async function withTempConfig(params: { cfg: unknown; run: () => Promise<void> }): Promise<void> {
  const prevConfigPath = process.env.NEXA_CONFIG_PATH;
  const prevDisableCache = process.env.NEXA_DISABLE_CONFIG_CACHE;

  const dir = await mkdtemp(path.join(os.tmpdir(), "nexa-plugin-http-auth-test-"));
  const configPath = path.join(dir, "nexa.json");
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.NEXA_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.NEXA_DISABLE_CONFIG_CACHE;
    } else {
      process.env.NEXA_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
