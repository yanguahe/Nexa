...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.NEXA_STATE_DIR;
  previousConfigPath = process.env.NEXA_CONFIG_PATH;
  previousSkipBrowserControl = process.env.NEXA_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.NEXA_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.NEXA_SKIP_CANVAS_HOST;
  previousBundledPluginsDir = process.env.NEXA_BUNDLED_PLUGINS_DIR;
  previousSkipChannels = process.env.NEXA_SKIP_CHANNELS;
  previousSkipProviders = process.env.NEXA_SKIP_PROVIDERS;
  previousSkipCron = process.env.NEXA_SKIP_CRON;
  previousMinimalGateway = process.env.NEXA_TEST_MINIMAL_GATEWAY;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "nexa-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.NEXA_STATE_DIR = path.join(tempHome, ".nexa");
  delete process.env.NEXA_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.NEXA_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.NEXA_SKIP_GMAIL_WATCHER = "1";
  process.env.NEXA_SKIP_CANVAS_HOST = "1";
  process.env.NEXA_SKIP_CHANNELS = "1";
  process.env.NEXA_SKIP_PROVIDERS = "1";
  process.env.NEXA_SKIP_CRON = "1";
  process.env.NEXA_TEST_MINIMAL_GATEWAY = "1";
  process.env.NEXA_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "nexa-test-no-bundled-extensions")
    : "nexa-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "nexa-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".nexa-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousConfigPath === undefined) {
      ...
      process.env.NEXA_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.NEXA_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.NEXA_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.NEXA_SKIP_GMAIL_WATCHER;
    } else {
      process.env.NEXA_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.NEXA_SKIP_CANVAS_HOST;
    } else {
      process.env.NEXA_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
    if (previousBundledPluginsDir === undefined) {
      delete process.env.NEXA_BUNDLED_PLUGINS_DIR;
    } else {
      process.env.NEXA_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
    }
    if (previousSkipChannels === undefined) {
      delete process.env.NEXA_SKIP_CHANNELS;
    } else {
      process.env.NEXA_SKIP_CHANNELS = previousSkipChannels;
    }
    if (previousSkipProviders === undefined) {
      delete process.env.NEXA_SKIP_PROVIDERS;
    } else {
      process.env.NEXA_SKIP_PROVIDERS = previousSkipProviders;
    }
    if (previousSkipCron === undefined) {
      delete process.env.NEXA_SKIP_CRON;
    } else {
      process.env.NEXA_SKIP_CRON = previousSkipCron;
    }
    if (previousMinimalGateway === undefined) {
      delete process.env.NEXA_TEST_MINIMAL_GATEWAY;
    } else {
      process.env.NEXA_TEST_MINIMAL_GATEWAY = previousMinimalGateway;
    }
  }
...
export async function startServerWithClient(
  ...
  const prev = process.env.NEXA_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.NEXA_GATEWAY_TOKEN = fallbackToken;
  }
