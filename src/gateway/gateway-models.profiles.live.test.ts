...
import type { NexaConfig, ModelProviderConfig } from "../config/types.js";
import { resolveNexaAgentDir } from "../agents/agent-paths.js";
...
import { ensureNexaModelsJson } from "../agents/models-config.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.NEXA_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.NEXA_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.NEXA_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<NexaConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.NEXA_AGENT_DIR,
  ...
  const hostAgentDir = resolveNexaAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "nexa-live-state-"));
  ...
  process.env.NEXA_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.nexa-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveNexaAgentDir();
  const sanitizedCfg: NexaConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "Nexa live tool probe (local, safe): " +
    ...
    process.env.NEXA_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.NEXA_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureNexaModelsJson(cfg);
      ...
      const rawModels = process.env.NEXA_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.nexa-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
