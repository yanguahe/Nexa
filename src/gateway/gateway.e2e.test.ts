...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      const prev = {
        ...
        skipBrowser: process.env.NEXA_SKIP_BROWSER_CONTROL_SERVER,
      };

      const { baseUrl: openaiBaseUrl, restore } = installOpenAiResponsesMock();

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "nexa-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "nexa");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = randomUUID();
      const nonceB = randomUUID();
      const toolProbePath = path.join(workspaceDir, `.nexa-tool-probe.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

      const configDir = path.join(tempHome, ".nexa");
      await fs.mkdir(configDir, { recursive: true });
      const configPath = path.join(configDir, "nexa.json");
      ...
      try {
        ...
        process.env.NEXA_CONFIG_PATH = prev.configPath;
        process.env.NEXA_GATEWAY_TOKEN = prev.token;
        process.env.NEXA_SKIP_CHANNELS = prev.skipChannels;
        process.env.NEXA_SKIP_GMAIL_WATCHER = prev.skipGmail;
        process.env.NEXA_SKIP_CRON = prev.skipCron;
        process.env.NEXA_SKIP_CANVAS_HOST = prev.skipCanvas;
        process.env.NEXA_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
      }
    },
  );
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "nexa-wizard-home-"));
    process.env.HOME = tempHome;
    ...
    try {
      ...
      await fs.rm(tempHome, { recursive: true, force: true });
      process.env.HOME = prev.home;
      process.env.NEXA_STATE_DIR = prev.stateDir;
      process.env.NEXA_CONFIG_PATH = prev.configPath;
      process.env.NEXA_GATEWAY_TOKEN = prev.token;
      process.env.NEXA_SKIP_CHANNELS = prev.skipChannels;
      process.env.NEXA_SKIP_GMAIL_WATCHER = prev.skipGmail;
