...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "nexa-cron-" });
}
...
async function writeSessionStore(
  ...
  const dir = path.join(home, ".nexa", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<NexaConfig> = {},
): NexaConfig {
  const base: NexaConfig = {
    agents: {
      defaults: {
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".nexa", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
