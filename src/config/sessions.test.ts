...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nexa-sessions-suite-"));
  });
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-nexa",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-nexa#general");
  });
  ...
  it("derives session transcripts dir from NEXA_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { NEXA_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.NEXA_STATE_DIR = "/custom/state";
    try {
