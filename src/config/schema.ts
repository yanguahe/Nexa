...
export type ConfigSchema = ReturnType<typeof NexaSchema.toJSONSchema>;
...
function buildBaseConfigSchema(): ConfigSchemaResponse {
  ...
  const schema = NexaSchema.toJSONSchema({
    target: "draft-07",
    unrepresentable: "any",
  });
  schema.title = "NexaConfig";
  const hints = mapSensitivePaths(NexaSchema, "", buildBaseHints());
  const next = {
