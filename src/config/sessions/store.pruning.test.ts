...
// Mock loadConfig so resolveMaintenanceConfig() never reads a real nexa.json.
...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nexa-pruning-suite-"));
});
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    ...
    storePath = path.join(testDir, "sessions.json");
    savedCacheTtl = process.env.NEXA_SESSION_CACHE_TTL_MS;
    process.env.NEXA_SESSION_CACHE_TTL_MS = "0";
    clearSessionStoreCacheForTest();
  ...
  afterEach(async () => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.NEXA_SESSION_CACHE_TTL_MS;
    } else {
      process.env.NEXA_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
