  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.nexa/logs/nexa.log" },
        hooks: {
          path: "~/.nexa/hooks.json5",
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".nexa", "logs", "nexa.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".nexa", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".nexa", "telegram.token"),
      );
      ...
      expect(cfg.agents?.list?.[0]?.agentDir).toBe(path.join(home, ".nexa", "agents", "main"));
