...
import type { NexaConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
...
import { compareNexaVersions } from "./version.js";
...
function coerceConfig(value: unknown): NexaConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as NexaConfig;
}
...
function stampConfigVersion(cfg: NexaConfig): NexaConfig {
...
function warnIfConfigFromFuture(cfg: NexaConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareNexaVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Nexa (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as NexaConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): NexaConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as NexaConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: NexaConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfig =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as NexaConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.NEXA_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.NEXA_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.NEXA_WATCH_SESSION === "string" &&
        deps.env.NEXA_WATCH_SESSION.trim().length > 0
          ? deps.env.NEXA_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.NEXA_WATCH_COMMAND === "string" &&
        deps.env.NEXA_WATCH_COMMAND.trim().length > 0
          ? deps.env.NEXA_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `NEXA_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.NEXA_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.NEXA_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): NexaConfig {
