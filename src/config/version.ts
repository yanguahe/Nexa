...
export function parseNexaVersion(raw: string | null | undefined): NexaVersion | null {
  if (!raw) {
    return null;
  }
...
export function compareNexaVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseNexaVersion(a);
  const parsedB = parseNexaVersion(b);
  if (!parsedA || !parsedB) {
    return null;
  }
