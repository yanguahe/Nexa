...
const ETC_NEXA_DIR = path.join(ROOT_DIR, "etc", "nexa");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "nexa.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcNexaPath(...parts: string[]) {
  return path.join(ETC_NEXA_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcNexaPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "nexa.json"))).toEqual({
      shared: true,
    });
  });
});
