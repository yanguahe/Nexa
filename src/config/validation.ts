...
import type { NexaConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: NexaConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: NexaConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = NexaSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as NexaConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as NexaConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as NexaConfig,
  };
}
