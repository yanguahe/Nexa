...
function setSlackDmPolicy(cfg: NexaConfig, dmPolicy: DmPolicy) {
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "Nexa";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for Nexa`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: NexaConfig, allowFrom: string[]): NexaConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
