...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    nexaHome: process.env.NEXA_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("NEXA_HOME", snapshot.nexaHome);
  restoreKey("NEXA_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked NEXA_HOME.
  delete process.env.NEXA_HOME;
  process.env.NEXA_STATE_DIR = path.join(base, ".nexa");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "nexa-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".nexa", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
