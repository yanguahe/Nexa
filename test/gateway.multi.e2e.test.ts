...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `nexa-e2e-${name}-`));
  const configDir = path.join(homeDir, ".nexa");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          NEXA_SKIP_BROWSER_CONTROL_SERVER: "1",
