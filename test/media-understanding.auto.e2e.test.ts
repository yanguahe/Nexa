...
import type { NexaConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("nexa-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("nexa-bin-sherpa-");
      const modelDir = await makeTempDir("nexa-sherpa-model-");
      ...
      const cfg: NexaConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("nexa-bin-whispercpp-");
      const modelDir = await makeTempDir("nexa-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("nexa-bin-gemini-");
      ...
      const cfg: NexaConfig = { tools: { media: { image: {} } } };
