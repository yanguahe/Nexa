---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Nexa deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Nexa cron scheduling for periodic checks, or version status checks on a machine running Nexa (laptop, workstation, Pi, VPS).
---
...
Assess and harden the host running Nexa, then align it to a user-defined risk tolerance without breaking access. Use Nexa security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Nexa changes the host firewall, SSH, or OS updates; it does not.
...
5. Nexa gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Nexa can access sensitive data.
...
### 2) Run Nexa security audits (read-only)

As part of the default read-only checks, run `nexa security audit --deep`. Only offer alternatives if the user requests them:

1. `nexa security audit` (faster, non-probing)
2. `nexa security audit --json` (structured output)

Offer to apply Nexa safe defaults (numbered):

1. `nexa security audit --fix`

Be explicit that `--fix` only tightens Nexa defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Nexa version/update status (read-only)

As part of the default read-only checks, run `nexa update status`.
...
- Credential hygiene notes (location of Nexa creds, prefer disk encryption)
...
After Nexa install or first hardening pass, run at least one baseline audit and version check:

- `nexa security audit`
- `nexa security audit --deep`
- `nexa update status`

Ongoing monitoring is recommended. Use the Nexa cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `nexa cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `nexa update status`
...
Before creating, `nexa cron list` and match on exact `name`. If found, `nexa cron edit <id> ...`.
If not found, `nexa cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `nexa update status` (preferred for source checkouts and channels)
2. `npm view nexa version` (published npm version)

## Nexa command accuracy

Use only supported commands and flags:

- `nexa security audit [--deep] [--fix] [--json]`
- `nexa status` / `nexa status --deep`
- `nexa health --json`
- `nexa update status`
- `nexa cron add|list|runs|run`

Do not invent CLI flags or imply Nexa enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Nexa compaction:
