...
read_when:
  - Designing workspace memory (~/.nexa/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Nexa integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.nexa/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.nexa/workspace` as canonical human-readable memory.
...
```
~/.nexa/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Nexa**, but keep a separable core library.

### Why integrate into Nexa?
...
- You want the agent itself to call the tools:
  - `nexa memory recall "…" --k 25 --since 30d`
  - `nexa memory reflect --since 7d`
...
Pragmatic take for `~/.nexa/workspace`:
