...
read_when:
  ...
  - Wiring external systems into Nexa
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your Nexa config directory (typically `~/.nexa/hooks/transforms`).
...
- `nexa webhooks gmail setup` writes `hooks.gmail` config for `nexa webhooks gmail run`.
