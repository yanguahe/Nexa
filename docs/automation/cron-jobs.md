...
- Jobs persist under `~/.nexa/cron/` so restarts don’t lose schedules.
...
nexa cron list
nexa cron run <job-id>
nexa cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.nexa/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `nexa cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, Nexa defaults to `announce`.
...
- Job store: `~/.nexa/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.nexa/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
nexa cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
nexa cron edit <jobId> --agent ops
nexa cron edit <jobId> --clear-agent
```
...
```bash
nexa cron run <jobId>
nexa cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
nexa cron edit <jobId> \
...
```bash
nexa system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`nexa system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `NEXA_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- Nexa applies exponential retry backoff for recurring jobs after consecutive errors:
