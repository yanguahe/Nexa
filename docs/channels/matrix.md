...
Matrix is an open, decentralized messaging protocol. Nexa connects as a Matrix **user**
...
1. Install the Matrix plugin:
   - From npm: `nexa plugins install @nexa/matrix`
   - From a local checkout: `nexa plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Nexa calls the same
     login endpoint, stores the access token in `~/.nexa/credentials/matrix/credentials.json`,
...
- On first connection, Nexa requests device verification from your other sessions.
...
`~/.nexa/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- On startup, Nexa resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
