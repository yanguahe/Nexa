...
Tlon is a decentralized messenger built on Urbit. Nexa connects to your Urbit ship and can
...
```bash
nexa plugins install @nexa/tlon
```

Local checkout (when running from a git repo):

```bash
nexa plugins install ./extensions/tlon
```
...
By default, Nexa blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `nexa message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Nexa replies in-thread.
