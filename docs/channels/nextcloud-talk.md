...
```bash
nexa plugins install @nexa/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
nexa plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Nexa" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `nexa pairing list nextcloud-talk`
  - `nexa pairing approve nextcloud-talk <CODE>`
