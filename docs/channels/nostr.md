...
read_when:
  - You want Nexa to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Nexa to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`nexa onboard`) and `nexa channels add` list optional channel plugins.
...
```bash
nexa plugins install @nexa/nostr
```

Use a local checkout (dev workflows):

```bash
nexa plugins install --link <path-to-nexa>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "nexa@example.com",
        "lud16": "nexa@example.com"
      }
    }
  }
}
