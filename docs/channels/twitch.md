...
read_when:
  - Setting up Twitch chat integration for Nexa
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Nexa connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
nexa plugins install @nexa/twitch
```

Local checkout (when running from a git repo):

```bash
nexa plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `NEXA_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "nexa", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use NEXA_TWITCH_ACCESS_TOKEN env var)
...
```bash
NEXA_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
