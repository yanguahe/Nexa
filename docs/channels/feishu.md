...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Nexa to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
nexa plugins install @nexa/feishu
```

Local checkout (when running from a git repo):

```bash
nexa plugins install ./extensions/feishu
```
...
If you just installed Nexa, run the wizard:
...
2. Configuring app credentials in Nexa
...
1. You already ran `nexa channels add` for Feishu
2. The gateway is running (`nexa gateway status`)
...
```bash
nexa pairing approve feishu <CODE>
```
...
2. Run `nexa logs --follow` and look for `chat_id`
...
2. Run `nexa logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `nexa gateway status`  | Show gateway status           |
| `nexa gateway install` | Install/start gateway service |
| `nexa gateway stop`    | Stop gateway service          |
| `nexa gateway restart` | Restart gateway service       |
| `nexa logs --follow`   | Tail gateway logs             |
...
5. Ensure the gateway is running: `nexa gateway status`
6. Check logs: `nexa logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.nexa/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.nexa/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
