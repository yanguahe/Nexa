---
summary: "Nexa on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Nexa on a Raspberry Pi
  - Running Nexa on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Nexa on Raspberry Pi

## Goal

Run a persistent, always-on Nexa Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Nexa features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled nexa
...
```bash
# Check logs
journalctl -u nexa --no-pager -n 100

# Common fix: rebuild
cd ~/nexa  # if using hackable install
npm run build
sudo systemctl restart nexa
```
