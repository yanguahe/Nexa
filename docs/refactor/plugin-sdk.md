...
- Publish as `nexa/plugin-sdk` (or export from core under `nexa/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `NexaPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: NexaConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: NexaConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: NexaConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: NexaConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: NexaConfig): string;
  };
};
```
...
- Introduce `nexa/plugin-sdk`.
- Add `api.runtime` to `NexaPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `nexaRuntime: ">=2026.2.0"`).
