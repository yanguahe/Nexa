---
summary: "Use MiniMax M2.1 in Nexa"
read_when:
  - You want MiniMax models in Nexa
...
```bash
nexa plugins enable minimax-portal-auth  # skip if already loaded.
nexa gateway restart  # restart if gateway is already running
nexa onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/nexa/nexa/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `nexa.json`:
...
- Use `nexa models list` and `nexa models set minimax/MiniMax-M2.1` to switch.
...
- Running `nexa configure` and selecting **MiniMax M2.1**, or
