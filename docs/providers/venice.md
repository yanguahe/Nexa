---
summary: "Use Venice AI privacy-focused models in Nexa"
read_when:
  - You want privacy-focused inference in Nexa
...
```bash
nexa onboard --auth-choice venice-api-key
```
...
```bash
nexa chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Nexa shows all available Venice models. Pick based on your needs:
...
```bash
nexa models set venice/claude-opus-45
nexa models set venice/llama-3.3-70b
```

List all available models:

```bash
nexa models list | grep venice
```

## Configure via `nexa configure`

1. Run `nexa configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Nexa automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
nexa chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
nexa chat --model venice/claude-opus-45

# Use uncensored model
nexa chat --model venice/venice-uncensored

# Use vision model with image
nexa chat --model venice/qwen3-vl-235b-a22b

# Use coding model
nexa chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `nexa models list` to see currently available models. Some models may be temporarily offline.
