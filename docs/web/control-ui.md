...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/nexa`)
...
If the page fails to load, start the Gateway first: `nexa gateway`.
...
you revoke it with `nexa devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.nexa/nexa.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Nexa
...
Nexa **blocks** Control UI connections without device identity.
