...
```bash
nexa tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `nexa tui --deliver`
...
- Check the Gateway logs: `nexa logs --follow`.
- Confirm the agent can run: `nexa status` and `nexa models status`.
...
- No agents in picker: check `nexa agents list` and your routing config.
