...
Nexa supports **camera capture** for agent workflows:
...
```bash
nexa nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
nexa nodes camera snap --node <id> --facing front
nexa nodes camera clip --node <id> --duration 3000
nexa nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`nexa.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `nexa` CLI to invoke camera commands on the macOS node.

Examples:

```bash
nexa nodes camera list --node <id>            # list camera ids
nexa nodes camera snap --node <id>            # prints MEDIA:<path>
nexa nodes camera snap --node <id> --max-width 1280
nexa nodes camera snap --node <id> --delay-ms 2000
nexa nodes camera snap --node <id> --device-id <id>
nexa nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
nexa nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
nexa nodes camera clip --node <id> --device-id <id>
nexa nodes camera clip --node <id> --no-audio
```

Notes:

- `nexa nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
nexa nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
