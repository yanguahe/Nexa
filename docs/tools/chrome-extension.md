---
summary: "Chrome extension: let Nexa drive your existing Chrome tab"
read_when:
...
The Nexa Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate nexa-managed Chrome profile.
...
Nexa then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Nexa release (npm package) as static files. There is no separate “build” step.

After upgrading Nexa:

- Re-run `nexa browser extension install` to refresh the installed files under your Nexa state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Nexa ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Nexa to control.
...
- `ON`: attached; Nexa can drive that tab.
...
Debugging: `nexa sandbox explain`
...
`nexa browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `nexa browser extension install` first to copy the extension to a stable location under your Nexa state directory.
...
- **This is not isolated** like the dedicated nexa-managed profile.
