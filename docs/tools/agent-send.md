---
summary: "Direct `nexa agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `nexa agent` (direct agent runs)

`nexa agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `nexa message --target`).
...
```bash
nexa agent --to +15555550123 --message "status update"
...
nexa agent --to +15555550123 --message "Summon reply" --deliver
