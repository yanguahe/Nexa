...
- **gateway host** → `nexa` process on the gateway machine
...
`~/.nexa/exec-approvals.json` directly.

CLI: `nexa approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
