---
summary: "Agent tool surface for Nexa (browser, canvas, nodes, message, cron) replacing legacy `nexa-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `nexa-*` skills
title: "Tools"
---

# Tools (Nexa)

Nexa exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `nexa-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `nexa.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Nexa logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:nexa`: all built-in Nexa tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`nexa node run`).
...
- Requires a Brave API key (recommended: `nexa configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated Nexa-managed browser.
...
- Quick smoke: `nexa nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `nexa gateway` restart in-place)
