...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `nexa sandbox explain`.
...
Legacy `agent.*` configs are migrated by `nexa doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=nexa-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${NEXA_STATE_DIR:-$HOME/.nexa}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
