...
Nexa features that can generate provider usage or paid API calls.
...
- `nexa status --usage` and `nexa channels list` show provider **usage windows**
...
Nexa can pick up credentials from:
...
`nexa models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
