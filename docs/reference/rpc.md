...
Nexa integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Nexa owns lifecycle when `channels.signal.autoStart=true`.
...
- Nexa spawns `imsg rpc` as a child process (legacy iMessage integration).
