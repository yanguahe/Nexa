...
所有日志配置都在 `~/.nexa/nexa.json` 的 `logging` 下。
...
- Nexa 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `nexa plugins enable diagnostics-otel` 启用插件。
...
- `nexa.tokens`（计数器，属性：`nexa.token`、`nexa.channel`、`nexa.provider`、`nexa.model`）
- `nexa.cost.usd`（计数器，属性：`nexa.channel`、`nexa.provider`、`nexa.model`）
- `nexa.run.duration_ms`（直方图，属性：`nexa.channel`、`nexa.provider`、`nexa.model`）
- `nexa.context.tokens`（直方图，属性：`nexa.context`、`nexa.channel`、`nexa.provider`、`nexa.model`）

消息流：

- `nexa.webhook.received`（计数器，属性：`nexa.channel`、`nexa.webhook`）
- `nexa.webhook.error`（计数器，属性：`nexa.channel`、`nexa.webhook`）
- `nexa.webhook.duration_ms`（直方图，属性：`nexa.channel`、`nexa.webhook`）
- `nexa.message.queued`（计数器，属性：`nexa.channel`、`nexa.source`）
- `nexa.message.processed`（计数器，属性：`nexa.channel`、`nexa.outcome`）
- `nexa.message.duration_ms`（直方图，属性：`nexa.channel`、`nexa.outcome`）

队列 + 会话：

- `nexa.queue.lane.enqueue`（计数器，属性：`nexa.lane`）
- `nexa.queue.lane.dequeue`（计数器，属性：`nexa.lane`）
- `nexa.queue.depth`（直方图，属性：`nexa.lane` 或 `nexa.channel=heartbeat`）
- `nexa.queue.wait_ms`（直方图，属性：`nexa.lane`）
- `nexa.session.state`（计数器，属性：`nexa.state`、`nexa.reason`）
- `nexa.session.stuck`（计数器，属性：`nexa.state`）
- `nexa.session.stuck_age_ms`（直方图，属性：`nexa.state`）
- `nexa.run.attempt`（计数器，属性：`nexa.attempt`）
...
- `nexa.webhook.error`
  - `nexa.channel`、`nexa.webhook`、`nexa.chatId`、`nexa.error`
- `nexa.message.processed`
  - `nexa.channel`、`nexa.outcome`、`nexa.chatId`、`nexa.messageId`、`nexa.sessionKey`、`nexa.sessionId`、`nexa.reason`
- `nexa.session.stuck`
  - `nexa.state`、`nexa.ageMs`、`nexa.queueDepth`、`nexa.sessionKey`、`nexa.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `nexa doctor`。
