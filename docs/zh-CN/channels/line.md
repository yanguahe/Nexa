...
LINE 通过 LINE Messaging API 连接到 Nexa。该插件作为 webhook 接收器在 Gateway 网关上运行，使用你的 channel access token + channel secret 进行身份验证。
...
```bash
nexa plugins install @nexa/line
```

本地检出（从 git 仓库运行时）：

```bash
nexa plugins install ./extensions/line
```
...
```bash
nexa pairing list line
nexa pairing approve line <CODE>
```
