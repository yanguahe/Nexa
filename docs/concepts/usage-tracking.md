...
- `/usage cost` in chats: local cost summary aggregated from Nexa session logs.
- CLI: `nexa status --usage` prints a full per-provider breakdown.
- CLI: `nexa channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
