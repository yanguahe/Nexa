...
read_when:
  - You need detailed behavior for nexa onboard
...
This page is the full reference for `nexa onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.nexa/nexa.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `nexa doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `nexa pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `nexa health`.
    - `nexa status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.nexa/.env` so launchd can read it.
...
- OAuth credentials: `~/.nexa/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.nexa/agents/<agentId>/agent/auth-profiles.json`

<Note>
Headless and server tip: complete OAuth on a machine with a browser, then copy
`~/.nexa/credentials/oauth.json` (or `$NEXA_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`nexa onboard`](/cli/onboard)
