---
summary: "How Nexa sandboxing works: modes, scopes, workspace access, and images"
title: Sandboxing
...
Nexa can run **tools inside Docker containers** to reduce blast radius.
...
- `"none"` (default): tools see a sandbox workspace under `~/.nexa/sandboxes`.
- `"ro"`: mounts the agent workspace read-only at `/agent` (disables `write`/`edit`/`apply_patch`).
- `"rw"`: mounts the agent workspace read/write at `/workspace`.

Inbound media is copied into the active sandbox workspace (`media/inbound/*`).
Skills note: the `read` tool is sandbox-rooted. With `workspaceAccess: "none"`,
Nexa mirrors eligible skills into the sandbox workspace (`.../skills`) so
...
Default image: `nexa-sandbox:bookworm-slim`
...
- Use `nexa sandbox explain` to inspect effective sandbox mode, tool policy, and fix-it config keys.
