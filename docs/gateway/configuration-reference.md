...
description: "Complete field-by-field reference for ~/.nexa/nexa.json"
---

# Configuration Reference

Every field available in `~/.nexa/nexa.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Nexa uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `nexa doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["nexa-dm"] },
      guilds: {
        "123456789012345678": {
...
Mattermost ships as a plugin: `nexa plugins install @nexa/mattermost`.
...
Nexa spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@nexa"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `nexa.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, Nexa auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Nexa assumes `anthropic` (deprecated).
...
- `none`: per-scope sandbox workspace under `~/.nexa/sandboxes`
    ...
    list: [
      {
        ...
        agentDir: "~/.nexa/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@nexa"] },
        sandbox: { mode: "off" },
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:nexa`   | All built-in tools (excludes provider plugins)                                           |
...
Nexa uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.nexa/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `NEXA_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `nexa onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `nexa onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `nexa onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `nexa onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `nexa onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `nexa onboard --auth-choice minimax-api`.
...
- Loaded from `~/.nexa/extensions`, `<workspace>/.nexa/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or NEXA_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `NEXA_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.nexa-dev` + port `19001`), `--profile <name>` (uses `~/.nexa-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.nexa/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-nexa-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `NEXA_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or NEXA_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__nexa__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__nexa__/a2ui/`
...
- Also serves A2UI at `/__nexa__/a2ui/`.
...
- Hostname defaults to `nexa`. Override with `NEXA_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.nexa/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `nexa dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.nexa/.env` (neither overrides existing vars).
...
- Legacy OAuth imports from `~/.nexa/credentials/oauth.json`.
...
- Default log file: `/tmp/nexa/nexa-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `nexa doctor --fix` can strip unknown keys).
