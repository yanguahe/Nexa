...
- During heartbeat runs, Nexa treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `nexa sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Nexa skips the heartbeat run entirely (no model call).
...
```bash
nexa system event --text "Check for urgent follow-ups" --mode now
```
