...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Nexa
...
Running Nexa via Node with `tsx` fails at startup with:

```
[nexa] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `nexa status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch nexa.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node nexa.mjs status` works on Node 25.
