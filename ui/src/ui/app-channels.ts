...
export async function handleWhatsAppStart(host: NexaApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: NexaApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: NexaApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: NexaApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: NexaApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: NexaApp): string {
...
function resolveGatewayHttpAuthHeader(host: NexaApp): string | null {
...
function buildGatewayHttpHeaders(host: NexaApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: NexaApp) {
...
export function handleNostrProfileToggleAdvanced(host: NexaApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: NexaApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: NexaApp) {
  const state = host.nostrProfileFormState;
