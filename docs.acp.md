...
This document describes how the Nexa ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`nexa acp` exposes an ACP agent over stdio and forwards prompts to a running
Nexa Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a Nexa Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `nexa acp` over stdio.

Example config:

```bash
nexa config set gateway.remote.url wss://gateway-host:18789
nexa config set gateway.remote.token <token>
```

Example run:

```bash
nexa acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
nexa acp --session agent:main:main
nexa acp --session agent:design:main
nexa acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “Nexa ACP” to start a thread.

## Execution Model

- ACP client spawns `nexa acp` and speaks ACP messages over stdio.
...
```bash
nexa acp --session agent:main:main
nexa acp --session-label "support inbox"
nexa acp --reset-session
```
...
`nexa acp` resolves the Gateway URL and auth from CLI flags or config:
