...
export async function handleNextcloudTalkInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as NexaConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as NexaConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as NexaConfig);
