...
Nexa extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
nexa channels login --channel zalouser
...
```bash
nexa message send --channel zalouser --target <threadId> --message "Hello from Nexa!"
```
...
```bash
nexa channels login --channel zalouser              # Login via QR
nexa channels login --channel zalouser --account work
nexa channels status --probe
nexa channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
nexa directory self --channel zalouser
nexa directory peers list --channel zalouser --query "name"
nexa directory groups list --channel zalouser --query "work"
nexa directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
nexa message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
nexa message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `nexa channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
nexa channels login --channel zalouser --account work
nexa message send --channel zalouser --account work --target <id> --message "Hello"
