...
metadata: { "nexa": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Nexa `sessions_spawn`
- **File I/O** == Nexa `read`/`write`
- **Remote fetch** == Nexa `web_fetch` (or `exec` with curl when POST is required)
