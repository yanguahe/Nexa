...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["nexaPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["nexa", "pollId"]) ??
    readNestedString(value, ["nexa", "poll", "id"]) ??
    readNestedString(value, ["data", "nexaPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "nexa", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        msteams: {
          ...
          value: { nexaPollId: pollId, pollId },
        },
      },
    ],
  };
