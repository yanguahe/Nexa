import type { NexaConfig, PluginRuntime } from "nexa/plugin-sdk";
...
export async function maybeCreateDynamicAgent(params: {
  ...
  const workspaceTemplate = dynamicCfg.workspaceTemplate ?? "~/.nexa/workspace-{agentId}";
  const agentDirTemplate = dynamicCfg.agentDirTemplate ?? "~/.nexa/agents/{agentId}/agent";
