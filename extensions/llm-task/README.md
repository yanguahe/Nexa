...
`nexa.invoke --each`) without adding new Nexa code per workflow.
...
This extension depends on Nexa internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Nexa extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.nexa/extensions` as a standalone plugin directory.
