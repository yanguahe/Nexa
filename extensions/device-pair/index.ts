...
import { approveDevicePairing, listDevicePairing } from "nexa/plugin-sdk";
...
function resolveGatewayPort(cfg: NexaPluginApi["config"]): number {
  const envRaw =
    process.env.NEXA_GATEWAY_PORT?.trim() || process.env.CLAWDBOT_GATEWAY_PORT?.trim();
  if (envRaw) {
...
async function resolveTailnetHost(api: NexaPluginApi): Promise<string | null> {
...
function resolveAuth(cfg: NexaPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: NexaPluginApi): Promise<ResolveUrlResult> {
