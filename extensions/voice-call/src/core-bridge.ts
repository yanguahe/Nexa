...
function resolveNexaRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.NEXA_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set NEXA_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveNexaRoot(), "dist", "extensionAPI.js");
