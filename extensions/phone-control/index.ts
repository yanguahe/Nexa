import type { NexaPluginApi, NexaPluginService } from "nexa/plugin-sdk";
...
function normalizeDenyList(cfg: NexaPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: NexaPluginApi["config"]): string[] {
...
export default function register(api: NexaPluginApi) {
  ...
  const timerService: NexaPluginService = {
