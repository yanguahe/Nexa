...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.NEXA_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "nexa-nostr-"));
  process.env.NEXA_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.NEXA_STATE_DIR?.trim() || env.NEXA_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        return path.join(homedir(), ".nexa");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.NEXA_STATE_DIR = previous;
    }
