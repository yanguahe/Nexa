...
# Build and bundle Nexa into a minimal .app we can open.
# Outputs to dist/Nexa.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Nexa.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Nexa"
BUNDLE_ID="${BUNDLE_ID:-ai.nexa.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/nexa/nexa/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Nexa/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :NexaBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :NexaGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Nexa"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Nexa"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Nexa"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Nexa" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Nexa/Resources/Nexa.icns" "$APP_ROOT/Contents/Resources/Nexa.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Nexa/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying NexaKit resources"
NEXAKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/NexaKit_NexaKit.bundle"
if [ -d "$NEXAKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/NexaKit_NexaKit.bundle"
  cp -R "$NEXAKIT_BUNDLE" "$APP_ROOT/Contents/Resources/NexaKit_NexaKit.bundle"
else
  echo "WARN: NexaKit resource bundle not found at $NEXAKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Nexa"
killall -q Nexa 2>/dev/null || true
