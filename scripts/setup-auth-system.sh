#!/bin/bash
# Setup Nexa Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'nexa-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Nexa message: Send warning via Nexa itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/nexa-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/nexa-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/nexa-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now nexa-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status nexa-auth-monitor.timer"
echo ""
echo "Quick commands:"
