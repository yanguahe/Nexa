...
docker run --rm -t \
  ...
  -e NEXA_LIVE_TEST=1 \
  -e NEXA_LIVE_MODELS="${NEXA_LIVE_MODELS:-${CLAWDBOT_LIVE_MODELS:-all}}" \
  -e NEXA_LIVE_PROVIDERS="${NEXA_LIVE_PROVIDERS:-${CLAWDBOT_LIVE_PROVIDERS:-}}" \
  -e NEXA_LIVE_MODEL_TIMEOUT_MS="${NEXA_LIVE_MODEL_TIMEOUT_MS:-${CLAWDBOT_LIVE_MODEL_TIMEOUT_MS:-}}" \
  -e NEXA_LIVE_REQUIRE_PROFILE_KEYS="${NEXA_LIVE_REQUIRE_PROFILE_KEYS:-${CLAWDBOT_LIVE_REQUIRE_PROFILE_KEYS:-}}" \
