...
const shouldBuild = (deps) => {
  if (deps.env.NEXA_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.NEXA_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[nexa] ${message}\n`);
};

const runNexa = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["nexa.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
