...
#
# Suggested cron: */30 * * * * /home/admin/nexa/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Nexa notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., nexa-alerts)
...
STATE_FILE="$HOME/.nexa/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Nexa if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use nexa
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Nexa to $NOTIFY_PHONE..."
            nexa send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Nexa is down. Run: ssh l36 '~/nexa/scripts/mobile-reauth.sh'" "urgent"
    exit 1
