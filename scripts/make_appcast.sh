...
ZIP=${1:?"Usage: $0 Nexa-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/nexa/nexa/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Nexa-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/nexa/nexa/releases/download/v${VERSION}/"}
