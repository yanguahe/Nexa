#!/usr/bin/env bash
# Reset Nexa like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${NEXA_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="Nexa.app/Contents/MacOS/Nexa"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/Nexa"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/Nexa"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/Nexa"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.nexa.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/nexa-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${NEXA_RESTART_LOG:-/tmp/nexa-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${NEXA_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.nexa/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  NEXA_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node nexa.mjs daemon install --force --runtime node"
      log "  node nexa.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.nexa/disable-launchagent"
      log ""
...
kill_all_nexa() {
  for _ in {1..10}; do
    ...
    if ! pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1 \
       ...
       && ! pgrep -x "Nexa" >/dev/null 2>&1; then
      return 0
    fi
    sleep 0.3
  done
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.nexa.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing Nexa instances"
kill_all_nexa
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product Nexa"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/Nexa.app" ]]; then
    APP_BUNDLE="/Applications/Nexa.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/Nexa.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/Nexa.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/Nexa.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set NEXA_APP_BUNDLE to your installed Nexa.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node nexa.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node nexa.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".nexa", "nexa.json"), "utf8");
