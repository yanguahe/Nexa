...
# Checks both Claude Code and Nexa auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
NEXA_AUTH="$HOME/.nexa/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    nexa models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    nexa_status=$(check_nexa_auth 2>/dev/null || true)

    claude_expires=0
    nexa_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        nexa_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        nexa_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$NEXA_AUTH" 2>/dev/null || echo "0")
    fi
    ...
    jq -n \
        ...
        '{
            ...
            nexa: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$nexa_status" == EXPIRED* ]] || [[ "$nexa_status" == MISSING* ]]; then
        echo "NEXA_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$nexa_status" == EXPIRING* ]]; then
        echo "NEXA_EXPIRING"
        exit 2
    else
        echo "OK"
...
echo ""
echo "Nexa Auth (~/.nexa/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$NEXA_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$NEXA_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'nexa doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active nexa >/dev/null 2>&1; then
    echo -e "Nexa service: ${GREEN}running${NC}"
else
    echo -e "Nexa service: ${RED}NOT running${NC}"
fi
