 ...
 * `nexa update`. Use it to verify the completion installation flow
...
\`nexa update\`, \`nexa doctor\`, and \`nexa onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Nexa Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`nexa completion --install\` later to enable.`));
    return;
  }
