...
INSTALL_URL="${NEXA_INSTALL_URL:-https://nexa.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${NEXA_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${NEXA_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="nexa"
PACKAGE_NAME="${NEXA_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (NEXA_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${NEXA_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${NEXA_INSTALL_LATEST_OUT:-}"
fi
