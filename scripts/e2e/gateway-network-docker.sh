...
IMAGE_NAME="nexa-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="nexa-net-e2e-$$"
GW_NAME="nexa-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "NEXA_GATEWAY_TOKEN=$TOKEN" \
	  -e "NEXA_SKIP_CHANNELS=1" \
	  -e "NEXA_SKIP_GMAIL_WATCHER=1" \
	  -e "NEXA_SKIP_CRON=1" \
	  -e "NEXA_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
